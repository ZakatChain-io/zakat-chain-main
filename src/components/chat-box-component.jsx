...
const ChatBoxComponent = ({ setShowChatBox }) => {
  const [textMessages, setTextMessages] = useState([
    {
      id: 1,
      message:
        "Hello there, I am ZakatChain's assistant. How can I help you today?",
      type: "assistant",
    },
    { id: 2, message: "I need help", type: "user" },
    {
      id: 2,
      message: "I am having issues with my zakat payment",
      type: "user",
    },
  ...
  const [chatBoxTextArea, setChatBoxTextArea] = useState({
    message: "",
    type: "user",
    id: conversationLength + 1,
  });
    ...
    <div className="absolute w-[90%] right-5 sm:right-10 -top-80 z-30 md:w-[40%] sm:w-[70%] ">
      <div className="px-6 pt-3 pb-6 bg-[#FF9606] flex justify-between ">
        <div className="flex relative">
          ...
          <div className="ml-20">
            <p className="text-white text-lg font-semibold">ZakatChain</p>
            <p className="text-white text-sm">Chat Box [Online]</p>
          </div>
        </div>
        <div className="flex gap-x-4 pt-1">
          <span className="cursor-pointer">
            <ChatBoxExpandIcon className="h-4 w-4" />
          </span>
          <span className="cursor-pointer">
            <ChatBoxMinimizeIcon className="h-4 w-4" />
          </span>
          <span className="cursor-pointer" onClick={onCollapseClick}>
            <ChatBoxCollapseIcon className="h-4 w-4" />
          </span>
        </div>
      </div>
      ...
      {textMessages.length === 0 ? (
        <div className="bg-white px-10 pt-10 pb-28 border-b border-black overflow-y-scroll h-60">
          <p className="text-center">
            We are here to help make your experience here on Zakat Chain,
            seamless. Ask Us anything.
          </p>
        </div>
      ) : (
        <div className="bg-white px-5 pt-10 pb-28 border-b border-black overflow-y-scroll h-60">
          {textMessages.map((message, index) => {
            if (message.type === "assistant") {
              ...
              return (
                <UserMessageComponent key={index} message={message.message} />
              );
            }
          })}
        </div>
      ...
      <div className="px-6 py-3 bg-white flex border-b rounded-b-xl">
        ...
        <div className="">
          <form>
            <textarea
              type="text"
              rows={1}
              onChange={handleChatBoxTextAreaChange}
              value={chatBoxTextArea.message}
        ...
        <div
          className="self-center ml-auto cursor-pointer"
